...
var getSubmission = exports.getSubmission = function getSubmission(name, id, formId) {
  ...
  return function (dispatch, getState) {
    ...
    var url = _lib.Formio.getProjectUrl() + '/' + (formId ? 'form/' + formId : name) + '/submission/' + id;
    var formio = new _lib.Formio(url);
...
var saveSubmission = exports.saveSubmission = function saveSubmission(name, data, formId) {
  ...
  return function (dispatch) {
    ...
    var formio = new _lib.Formio(_lib.Formio.getProjectUrl() + '/' + (formId ? 'form/' + formId : name) + '/submission' + (id ? '/' + id : ''));

    formio.saveSubmission(data).then(function (result) {
      var url = _lib.Formio.getProjectUrl() + '/' + (formId ? 'form/' + formId : name) + '/submission/' + result._id;
...
var deleteSubmission = exports.deleteSubmission = function deleteSubmission(name, id, formId) {
  var done = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : function () {};
  return function (dispatch, getState) {
    var formio = new _lib.Formio(_lib.Formio.getProjectUrl() + '/' + (formId ? 'form/' + formId : name) + '/submission/' + id);
